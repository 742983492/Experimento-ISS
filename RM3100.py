...
def printlog(message, maglog):
    """Log messages to both stdout and a specified log file.

    Args:
        message (str): The message to log.
        maglog (file): File object for writing log messages.

    Returns:
        bool: True if logging is successful, False otherwise.

    Notes:
        - Automatically includes a timestamp in the log messages.
    """
    timestamp = time.strftime("%Y/%m/%d %H:%M:%S", time.gmtime())
    formatted_message = f"{timestamp}\t{message}"
    try:
        print(formatted_message)
        print(formatted_message, file=maglog)
        return True
...
class RM3100:
    ...
    def __init__(self, userbus, i2c_address, maglog):
        ...
        try:
            ...
            printlog(f'{hex(i2c_address)}\tBus {userbus} initialized for magnetometer', maglog)
        except Exception as error:
            printlog(f'{hex(i2c_address)}\tBus {userbus} failed to initialize (SMBus error).\t{error}', maglog)

    def read8(self, reg, maglog):
        ...
        try:
            ...
            printlog(f'Failed to read8 {hex(reg)} register. {error}', maglog)
            return False
    ...
    def read16(self, reg, maglog):
        ...
        try:
            ...
            printlog(f'Failed to read16 {hex(reg)} register. {error}', maglog)
            return False

    def write8(self, reg, value, maglog):
        ...
        try:
            ...
            # Log the error and return False to indicate failure
            printlog(f'Failed to write8 {hex(reg)} register. {error}', maglog)
            return False

    def write16(self, reg, value, maglog):
        ...
        try:
            ...
            # Log the error and return False to indicate failure
            printlog(f'Failed to write16 {hex(reg)} register. {error}', maglog)
            return False

    # def read_measurements(self, maglog):
    #     """Read the X, Y, Z measurement values from the magnetometer.

    #     Args:
    #         maglog (file): Log file object for logging any errors.

    #     Returns:
    #         dict or bool: A dictionary containing the X, Y, Z measurements, or False if the read failed.
    #     """
    #     try:
    #         # measurements = [self.i2cbus.read_byte_data(self.i2c_address, 0xA4 & ~0x80)]
    #         # measurements.extend(self.i2cbus.read_byte(self.i2c_address) for _ in range(8))
    ...
    #         x = self._convert_measurement(measurements[0:3])
    #         y = self._convert_measurement(measurements[3:6])
    #         z = self._convert_measurement(measurements[6:9])

    #         if measurements[0]>=64:
    #             xold = int.from_bytes([255,measurements[0],measurements[1],measurements[2]],'big',signed=True)
    #         else:
    #             xold = int.from_bytes([0,measurements[0],measurements[1],measurements[2]],'big',signed=True)
                
    #         if measurements[3] >=64:
    #             yold = int.from_bytes([255,measurements[3],measurements[4],measurements[5]],'big',signed=True)
    #         else:
    #             yold = int.from_bytes([0,measurements[3],measurements[4],measurements[5]],'big',signed=True)
                
    #         if measurements[6] >=64:
    #             zold = int.from_bytes([255,measurements[6],measurements[7],measurements[8]],'big',signed=True)
    #         else:
    #             zold = int.from_bytes([0,measurements[6],measurements[7],measurements[8]],'big',signed=True)

    #         if x != xold:
    #             printlog(f'x={x} xold={xold} ARE DIFFERENT \n', maglog)
    #         if y != yold:   
    #             printlog(f'y={y} yold={yold} ARE DIFFERENT \n', maglog)
    #         if z != zold:
    #             printlog(f'z={z} zold={zold} ARE DIFFERENT \n', maglog)

    #         return {'x': x, 'y': y, 'z': z}
    #     except Exception as error:
    #         printlog(f'Failed to read measurements: {error}', maglog)
    #         return False
        
    # def read_measurements(self, maglog):
    #     """Read X, Y, Z magnetic field measurements.

    #     Args:
    #         maglog (file): Log file for capturing errors.

    #     Returns:
    #         dict: Contains 'x', 'y', 'z' magnetic field values.
    #     """
    #     try:
    #         measurements = [
    #             self.i2cbus.read_byte_data(self.i2c_address, 0xA4 & ~0x80),
    #             *(self.i2cbus.read_byte(self.i2c_address) for _ in range(8))
    #         ]
    ...
    def check_measurement(self, maglog):
        ...
        """
        value = self.read8(0xB4, maglog)
        if value is False:
            printlog("Failed to read register 0xB4. Measurement check failed. No connection to PNI.", maglog)
            return False
    ...
    def read_frequency(self, maglog):
        ...
        """
        rfrq = self.read8(0x8B, maglog)
        if rfrq is not False:
            printlog(f'{hex(self.i2c_address)}\tFrequency={rfrq}', maglog)
        return rfrq
...
def launch(userbus, addr, frq, cycles, maglog):
    ...
    """
    mag = RM3100(userbus, addr, maglog)
    ...
    reg = mag.read8(RM3100_REGISTER_TEST, maglog)
    if reg is False:
        printlog("Failed to read register 0xB6. Status test failed. No connection to PNI.", maglog)
        raise Exception("Failed to read register 0xB6 (Test). Status test failed. No connection to PNI.")

    mag.write8(RM3100_REGISTER_TMRC, frq, maglog)  # Set custom frequency
    mag.write16(RM3100_REGISTER_CMX, cycles, maglog)
    mag.write16(RM3100_REGISTER_CMY, cycles, maglog)
    mag.write16(RM3100_REGISTER_CMZ, cycles, maglog)

    # Read back configuration for verification
    rccx = mag.read16(RM3100_REGISTER_RCMX, maglog)  # Read cycle count for X-axis
    rccy = mag.read16(RM3100_REGISTER_RCMY, maglog)  # Read cycle count for Y-axis
    rccz = mag.read16(RM3100_REGISTER_RCMZ, maglog)  # Read cycle count for Z-axis
    rfrq = mag.read8(RM3100_REGISTER_RTMRC, maglog)  # Read frequency

    # Log configuration details
    printlog(hex(addr)+ '\t' + f'Frequency={rfrq}',maglog)
    printlog(hex(addr)+ '\t' + f'Cycle count X={rccx}',maglog)
    printlog(hex(addr)+ '\t' + f'Cycle count Y={rccy}',maglog)
    printlog(hex(addr)+ '\t' + f'Cycle count Z={rccz}\n',maglog)

    # Set continuous measurement mode
    mag.write8(RM3100_REGISTER_CMM, 0x79, maglog)

    return mag


def measure_and_save_cont_MAG(mags, duration, maglog, save_folder):
    """
    ...
    Args:
        ...
        maglog (file): Log file for recording events and progress.
    ...
    while time.time() <= finish + pretime:
        for i, mag in enumerate(mags):
            if mag.check_measurement(maglog) and counters[i]<worstcounter:  # Check if data is ready and counter is less than worst case
                try:
                    # Perform measurement
                    timestamp = time.time()
                    ...
                    if counters[i] % sampleprint == 0:
                        printlog(f"{hex(mag.i2c_address)}\t{counters[i]}\tX={x}\t\tY={y}\t\tZ={z}", maglog)
                    ...
                    printlog(f"Error during measurement or file writing for magnetometer {hex(mag.i2c_address)}: {e}", maglog)
...
def measure_and_save_cont_MAGTEMP(mags, mcps, duration, maglog, save_folder):
    """
    Perform continuous measurement with multiple magnetometers and save results in real-time to CSV files.
    ...
    # Initialize file paths, counters, and CSV writers
    for (mag, temp) in (zip(mags, mcps)):
        magaddr = hex(mag.i2c_address)
        tempaddr = hex(temp.i2c_address)
        number_files=next_numeric_prefix(save_folder,mag_id=magaddr)
        file_path = os.path.join(save_folder, f"{number_files}_mag_{magaddr}_temp_{tempaddr}_{time.strftime('%Y%m%d_%H%M%S', time.gmtime(start))}.csv")
        file_paths.append(file_path)
        counters.append(0)  # Initialize counter to start from 0
        ...
        f = open(file_path, 'w', newline='')
        writer = csv.writer(f)
        writer.writerow(["Timestamp", "Counter", "X", "Y", "Z", "T"])  # Header row
        writers.append((f, writer))  # Store file object and writer together
        printlog(f"Initialized CSV for {magaddr}: {file_path}\n", maglog)

    pretime = time.time() - start

    # Main measurement loop
    while time.time() <= finish + pretime:
        for i, (mag, temp) in enumerate(zip(mags, mcps)):
            if mag.check_measurement(maglog) and counters[i]<worstcounter:  # Check if data is ready and counter is less than worst case
                try:
                    # Perform measurement
                    timestamp = time.time()
                    ...
                    t=temp.read_temp(maglog)

                    # Write the measurement directly to the CSV file
                    _, writer = writers[i]  # Retrieve writer for the current magnetometer
                    writer.writerow([timestamp, counters[i], x, y, z, t])

                    # Log the sample if it matches the print interval
                    if counters[i] % sampleprint == 0:
                        printlog(f"{hex(mag.i2c_address)}\t{counters[i]}\tX={x}\tY={y}\tZ={z}\tT={t}", maglog)
...
def measure_and_save_cont_test(mags, duration, maglog, save_folder):
    """
    Perform continuous measurement with multiple magnetometers and save results in real-time to CSV files.
    ...
    while time.time() <= finish + pretime:
        for i, mag in enumerate(mags):
            try:
                ...
                current=time.time()
                while time.time() - current < 0.0005:
                    a=1
...
def measure_and_save_cont_IO(mags, duration, maglog, save_folder):
    """
    Perform continuous measurement with multiple magnetometers and save results in real-time to CSV files.
    ...
    while time.time() <= finish + pretime:
        for i, mag in enumerate(mags):
            if mag.check_measurement(maglog) and counters[i]<worstcounter:  # Check if data is ready
                try:
                    # Perform measurement
                    timestamp = time.time()
                    mag_data = mag.read_measurements(maglog)
                    ...
                    # Open the file, write the measurement, and close it
